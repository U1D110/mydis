...
fn tcp_listener_accepts_and_echoes() -> io::Result<()> {
    ...
    let handle = std::thread::spawn(move || {
        ...
        client
            ...
            .expect("Jeff failed to write to server");
        let mut buf = [0u8; 28];
        client
            .read(&mut buf)
            .expect("Jeff failed to read from server");
        ...
        client
            ...
            .expect("Bort failed to write to server");
        let mut buf = [0u8; 28];
        client
            .read(&mut buf)
            .expect("Bort failed to read from server");
    ...
    for i in 0..2 {
        ...
        assert_eq!(
            events.iter().count(),
            1,
            "Expected exactly one readable event {i}"
        );
        let mut buf = [0u8; 28];
        ...
        poll.wait(&mut events, -1)?;
        assert!(
            events.iter().any(|e| e.rdhup()),
            "Expected EPOLLRDHUP from client {i} disconnect"
        );
    }
