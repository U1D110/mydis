# Project Roadmap (Revised) — In-Memory Key-Value Server

## Project Goals

Two learning objectives remain the **spine** of this project and the reason it
exists:
...
**Revised terminus:** once the runtime exists, the server evolves into a
**usable Redis-like key-value server** with an intentionally limited but growing
command vocabulary. Redis remains a reference point; feature-completeness is now
an explicit *later* destination rather than an anti-goal. Feature breadth is
layered on **only after** the runtime lands — the networking and runtime phases
come first because they are the point.

**Nothing is permanently excluded.** The original "Non-Goals" list (data
structures, transactions, pub/sub, eviction, INFO, replication, TLS, cluster) is
now sequenced as **deferred future phases** (Phase 9+), ordered by dependency and
by how much each stresses the runtime.

## What Changed Since the Original Roadmap

- **Progress:** Phases 0–5 complete; Phase 6 nearly complete (only graceful
  shutdown remains).
- **Non-Goals → Deferred:** every former hard exclusion is now a planned later
  phase.
- **Design divergences** recorded formally below. Landed since the original plan:
  full `std::os::fd` adoption, the generation-counter fd-reuse fix, the
  `BTreeSet` expiry structure, and AOF command normalization.

## Guiding Principle (retained + extended)
...
feel painful enough that the runtime is *inevitable*. Extended: once the runtime
exists, each new command family or data type becomes a new kind of task or
storage concern that exercises the runtime and re-tests the parser/DB separation.

## Architectural Invariants (unchanged, still binding)
...
- **Divergence:** chose `BTreeSet` with direct removal over the originally
  proposed `BinaryHeap` + tombstones — removal is O(log n) and exact, so
  tombstones are unnecessary at this scale.
...
Purpose unchanged: force the first operation that cannot complete synchronously
within one event-loop iteration — the conceptual seed of the runtime.
...
### Design divergence recorded here

**Completion transport.** The original Phase 6.5 sketch put a
`Receiver<io::Result<()>>` inside each connection's `FlushState`. The actual
design uses a **centralized** shared `Receiver<Completion>` in `Aof` plus a single
`eventfd`, dispatching completions to connections by fd (validated by generation).
This is *ahead* of the sketch — it already has the reactor/waker shape Phase 7
formalizes.

---

## Phase 6.5 — Make the Implicit Explicit (REVISED — supersedes the Receiver sketch)
...
on top of the **actual** centralized transport:
...
Everything below was formerly a hard Non-Goal and is now **deferred, not
excluded**. Each phase builds on the finished runtime and must preserve the
architectural invariants (parser/DB separation, structured responses,
single-threaded). Ordering is by dependency and runtime stress.
